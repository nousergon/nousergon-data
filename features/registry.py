...
CATALOG: list[FeatureEntry] = [
    ...
    FeatureEntry("earnings_surprise_pct", "alternative", "Most recent quarterly EPS surprise %", source="finnhub", refresh="weekly", units='decimal pct (`_pct` suffix)', formula='FMP earnings surprise %', consumers='predictor', display_order=0),
    FeatureEntry("days_since_earnings", "alternative", "Days since last earnings (0-1, capped 90d)", source="finnhub", refresh="weekly", units='0–1 normalized (days / 90)', formula='`days / 90.0`', consumers='predictor', display_order=1),
    FeatureEntry("eps_revision_4w", "alternative", "4-week cumulative EPS revision %", source="finnhub", refresh="weekly", units='decimal pct (bare-named convention)', formula='FMP 4-week EPS revision %', consumers='predictor', display_order=2),
    FeatureEntry("revision_streak", "alternative", "Consecutive weeks of same-direction revisions", source="finnhub", refresh="weekly", units='count (bare-named convention — integer count)', formula='Consecutive same-direction-revision weeks', consumers='predictor', display_order=3),
    ...
    FeatureEntry("pe_ratio", "fundamental", "Trailing P/E ratio, normalized (PE / 30)", source="finnhub", refresh="quarterly", units='normalized (PE / 30)', formula='trailing P/E normalized', consumers='predictor', display_order=0),
    FeatureEntry("pb_ratio", "fundamental", "Price-to-book ratio, normalized (PB / 5)", source="finnhub", refresh="quarterly", units='normalized (PB / 5)', formula='price-to-book normalized', consumers='predictor', display_order=1),
    FeatureEntry("debt_to_equity", "fundamental", "Total debt / total equity, normalized (D/E / 2)", source="finnhub", refresh="quarterly", units='normalized (D/E / 2)', formula='total debt / total equity normalized', consumers='predictor', display_order=2),
    FeatureEntry("revenue_growth_yoy", "fundamental", "Year-over-year revenue growth (decimal)", source="finnhub", refresh="quarterly", units='decimal pct (bare-named convention)', formula='year-over-year revenue growth', consumers='predictor', display_order=3),
    FeatureEntry("fcf_yield", "fundamental", "Free cash flow / market cap (decimal)", source="finnhub", refresh="quarterly", units='decimal pct (bare-named convention)', formula='FCF / market cap', consumers='predictor', display_order=4),
    FeatureEntry("gross_margin", "fundamental", "Gross profit / revenue (0-1)", source="finnhub", refresh="quarterly", units='0–1 fraction (bare-named convention)', formula='gross profit / revenue', consumers='predictor', display_order=5),
    FeatureEntry("roe", "fundamental", "Return on equity (decimal)", source="finnhub", refresh="quarterly", units='decimal pct (bare-named convention)', formula='return on equity', consumers='predictor', display_order=6),
    FeatureEntry("current_ratio", "fundamental", "Current assets / current liabilities, normalized (CR / 3)", source="finnhub", refresh="quarterly", units='normalized (CR / 3)', formula='current assets / current liabilities normalized', consumers='predictor', display_order=7),
    # ── Phase 3a of attractiveness-pillars-260520 — Growth + Stewardship pillar substrate ──
    FeatureEntry("revenue_growth_3y", "fundamental", "3-year revenue CAGR (decimal); Growth pillar input", source="finnhub", refresh="quarterly", units='decimal pct CAGR (bare-named convention)', formula='3y revenue CAGR', consumers='predictor', display_order=8),
    FeatureEntry("eps_growth_3y", "fundamental", "3-year EPS CAGR (decimal); Growth pillar input", source="finnhub", refresh="quarterly", units='decimal pct CAGR (bare-named convention)', formula='3y EPS CAGR', consumers='predictor', display_order=9),
    FeatureEntry("payout_ratio", "fundamental", "TTM dividends / net income (0-2 clipped); Stewardship pillar input — retention rate = 1 - payout drives reinvestment", source="finnhub", refresh="quarterly", units='0–2 clipped ratio', formula='TTM dividends / net income', consumers='predictor', display_order=10),
    FeatureEntry("dividend_yield", "fundamental", "Indicated annual dividend yield (decimal, 0-0.2 clipped); Stewardship pillar input", source="finnhub", refresh="quarterly", units='decimal pct (bare-named convention)', formula='indicated annual dividend yield', consumers='predictor', display_order=11),
    FeatureEntry("capex_growth_5y", "fundamental", "5-year CAPEX growth (decimal); Stewardship pillar input — reinvestment intensity proxy", source="finnhub", refresh="quarterly", units='decimal pct (bare-named convention)', formula='5y CAPEX growth', consumers='predictor', display_order=12),
    ...
    FeatureEntry("mom_12_1_pct_zscore", "factor_loading", "Cross-sectional z-score of mom_12_1_pct (winsorized ±3σ). Barra MOMENTUM (long-horizon / RSTR) loading — trailing 12-month return excluding the most recent month, the horizon the Jegadeesh-Titman momentum premium is defined over.", source="derived", refresh="daily", units='z-score', formula='Cross-sectional z of `mom_12_1_pct`, ±3σ winsorized', consumers='scanner (mom_12_1_sleeve challenger arm)', display_order=2),
