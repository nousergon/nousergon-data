...
_SPEC_FIELDS = frozenset(
    {
        ...
        # the grammar and `check_freshness` applies the DST-aware re-grade
        # itself, so the monitor threads the raw string straight through
        # rather than parsing and re-grading it a second time here.
        "deadline_local",
    }
)
...
def load_registry_with_recovery(
    ...
    ``deadline_local`` (alpha-engine-config-I10805) is NOT a parallel map
    here as of I10829 phase 2: it is a native ``ArtifactSpec`` field on the
    pinned nousergon-lib (>=0.124.129), so ``_SPEC_FIELDS`` threads the raw
    registry string straight into the constructor and ``check_freshness``
    applies the DST-aware re-grade itself.
...
def _run_probe_pass(
    ...
    aws_clients: dict[str, Any] = {}
    for spec in specs:
        # alpha-engine-config-I10805 / I10829 phase 2 — a row declaring
        # `deadline_local` is graded against that wall-clock local deadline
        # rather than the substrate's cadence floor; `check_freshness` (the
        # pinned nousergon-lib >=0.124.129) applies the DST-aware re-grade
        # itself, so every downstream consumer (driver attribution, episode
        # identity, `_alert_decision`, `check_results.json`, the digest)
        # sees the ONE resulting verdict with no second re-grade step here.
