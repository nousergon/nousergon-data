#!/usr/bin/env bash
# deploy.sh — Create or update the telos-runner-dispatcher Lambda.
#
# WHY: mirrors nousergon/alpha-engine-config's self-hosted-runner dispatcher
# (alpha-engine-config-I2572) verbatim in mechanism, re-namespaced for telos.
# A live billing-usage audit (2026-07-17) found telos among the largest
# non-alpha-engine-config private-repo GHA-minute consumers (~1,225 min in
# July alone, ci.yml's 4-way Python matrix + web build) after the org
# crossed its 3,000-min included-Actions-minute line for the month. This
# Lambda receives GitHub's `workflow_job` webhook directly
...
# (scoped to telos-runner-executor-role — a NEW, dedicated role
# a sibling agent is creating in telos) + ssm:SendCommand +
...
#        /telos/runner/webhook_secret  (random string, shared
#          with the GitHub webhook config below)
#        /telos/runner/github_pat  (fine-grained PAT, owner=
#          nousergon, repo=telos, Administration: Read and
...
#        /telos/runner/github_read_pat  (SEPARATE fine-grained
#          PAT, owner=nousergon, repo=telos, Actions: Read
...
#   2. Register the GitHub webhook on nousergon/telos:
#        event: workflow_job, content type: json, secret: (the same value as
#        the SSM param above), URL: this script's printed Function URL.
#      Can be done via `gh api repos/nousergon/telos/hooks`.
#
# Usage:
#   bash .../telos-runner-dispatcher/deploy.sh             # update code only (also the CI auto-deploy path)
#   bash .../telos-runner-dispatcher/deploy.sh --bootstrap # operator-only: create/update the IAM role + Lambda + Function URL + reconcile schedule
#   bash .../telos-runner-dispatcher/deploy.sh --apply-iam # re-apply iam-policy.json only (no bootstrap side effects, config#2825)
#   bash .../telos-runner-dispatcher/deploy.sh --dry-run   # show actions, do not apply
#   bash .../telos-runner-dispatcher/deploy.sh --smoke     # invoke the WORKER phase once with a synthetic job id (fires a REAL spot box)
#   bash .../telos-runner-dispatcher/deploy.sh --reconcile-test # invoke the RECONCILE phase once directly (read-only unless it finds a genuinely stale job)

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/../_shared/apply_iam_policy.sh"
FUNCTION_NAME="telos-runner-dispatcher"
ROLE_NAME="telos-runner-dispatcher-role"
POLICY_NAME="telos-runner-dispatcher-policy"
...
LAMBDA_ENV_BOOTSTRAP='Variables={LOG_LEVEL=INFO,TELOS_RUNNER_DISPATCH_ENABLED=true}'
...
# instead of silently running the real deploy path (telos-
...
if $BOOTSTRAP; then
  ...
  if ! $DRY_RUN; then
    ...
    echo "  -> register as the GitHub webhook target for nousergon/telos"
    echo "     (event: workflow_job, content type: json, secret: matches"
    echo "     /telos/runner/webhook_secret) — see this script's"
  ...
  RECONCILE_SCHED_ROLE_NAME="telos-runner-reconcile-scheduler-role"
  RECONCILE_SCHED_POLICY_NAME="invoke-telos-runner-dispatcher"
  ...
  RECONCILE_SCHED_NAME="telos-runner-reconcile"
...
echo "Updating Lambda environment (preserving operator-owned TELOS_RUNNER_DISPATCH_ENABLED)..."
CURRENT_DISPATCH=$(preserve_env_flag "${FUNCTION_NAME}" "${REGION}" TELOS_RUNNER_DISPATCH_ENABLED true)
LAMBDA_ENV="Variables={LOG_LEVEL=INFO,TELOS_RUNNER_DISPATCH_ENABLED=${CURRENT_DISPATCH}}"
...
if $SMOKE; then
  ...
  echo "⚠ this fires a REAL spot box + REAL telos_runner_spot_bootstrap.sh run."
  RESP=$(mktemp)
  trap "rm -f '${RESP}'" EXIT
  aws lambda invoke \
    --function-name "${FUNCTION_NAME}" \
    --payload '{"telos_runner_job_id":"smoke-test-0"}' \
