"""vires-runner-dispatcher — launch an EPHEMERAL GitHub Actions
self-hosted runner on a dedicated EC2 spot box, once per queued CI/validation
job in nousergon/vires. Mirrors nousergon/alpha-engine-config's
self-hosted-runner dispatcher (alpha-engine-config-I2572) verbatim in
mechanism, re-namespaced for vires.

WHY: a hard GHA audit (2026-07-14) found vires's own CI/
...
`handler` for the branch):
  1. WEBHOOK RECEIVER phase: GitHub calls this Lambda's Function URL directly
     on every `workflow_job` event for vires (repo-level
     webhook, event type `workflow_job`). Verifies the HMAC signature, filters
     to `action=queued` + our `vires-spot` label, then
  ...
  2. WORKER phase (the self-invoked async call): does the actual dispatch —
     ...
     command that clones vires and `exec`s
     `infrastructure/vires_runner_spot_bootstrap.sh` (built by a sibling
     agent in vires) — which installs+registers the actual
  ...
  3. RECONCILE phase (EventBridge Scheduler, ~every 60s — config-I2653): a
     ...
     longer than `VIRES_RUNNER_RECONCILE_STALE_SECONDS` with no in-flight
...
total fleet size. Both live in `_launch_vires_runner_spot()` (the single
...
a job at/over `VIRES_RUNNER_MAX_ATTEMPTS_PER_JOB` launches (default 3,
...
capped at `VIRES_RUNNER_MAX_FLEET` running+pending boxes (default 6 = 12
...
vires_runner_launches` custom metric + CloudWatch alarm
(infrastructure/setup_vires_runner_launch_rate_alarm.sh) is the
...
CONCURRENCY LOCK: keyed on `Name=vires-runner-spot` +
`vires-runner-job-id=<workflow_job.id>` — one job, one box, 1:1 (unlike
...
IAM PROFILE: `vires-runner-executor-profile`, a NEW dedicated
instance profile (infrastructure/iam/vires-runner-executor-role-*.json in
vires) scoped to read exactly one SSM param (the runner-
...
with Administration:write on vires exists in SSM — see
...
# Kill-switch: VIRES_RUNNER_DISPATCH_ENABLED=false disables the launch
# without touching the GitHub webhook wiring — mirrors every other fleet
# dispatcher's safety valve. Default ON.
DISPATCH_ENABLED = os.environ.get("VIRES_RUNNER_DISPATCH_ENABLED", "true").lower() == "true"

TARGET_REPO_FULL_NAME = "nousergon/vires"
TARGET_LABEL = "vires-spot"
...
INSTANCE_TYPES = [
    ...
    for t in os.environ.get(
        "VIRES_RUNNER_INSTANCE_TYPES", "t3.medium,t3a.medium,t2.medium"
    ).split(",")
    if t.strip()
]
SUBNETS = [
    ...
    for s in os.environ.get(
        "VIRES_RUNNER_SUBNETS",
        "subnet-a61ec0fb,subnet-1e58307a,subnet-789d3857,"
        "subnet-c670118d,subnet-7cff7c43,subnet-e07166ec",
    ).split(",")
    if s.strip()
]
AMI_ID = os.environ.get("VIRES_RUNNER_AMI_ID", "ami-0c421724a94bba6d6")  # Amazon Linux 2023 x86_64
KEY_NAME = os.environ.get("VIRES_RUNNER_KEY_NAME", "alpha-engine-key")
SECURITY_GROUP = os.environ.get("VIRES_RUNNER_SECURITY_GROUP", "sg-03cd3c4bd91e610b0")
IAM_PROFILE = os.environ.get(
    "VIRES_RUNNER_IAM_PROFILE", "vires-runner-executor-profile"
)
VOLUME_SIZE_GB = int(os.environ.get("VIRES_RUNNER_VOLUME_SIZE_GB", "30"))

VIRES_RUNNER_TAG_NAME = "vires-runner-spot"
VIRES_RUNNER_JOB_ID_TAG_KEY = "vires-runner-job-id"
...
# inside _launch_vires_runner_spot (the single chokepoint both the reactive
...
#      running+pending vires-runner boxes >= MAX_FLEET, regardless of
...
VIRES_RUNNER_MAX_ATTEMPTS_PER_JOB = int(os.environ.get("VIRES_RUNNER_MAX_ATTEMPTS_PER_JOB", "3"))
...
VIRES_RUNNER_ATTEMPT_LOOKBACK_SECONDS = int(
    os.environ.get("VIRES_RUNNER_ATTEMPT_LOOKBACK_SECONDS", "3600")
)
...
VIRES_RUNNER_MAX_FLEET = int(os.environ.get("VIRES_RUNNER_MAX_FLEET", "6"))
...
# silently again).
VIRES_RUNNER_BOOTSTRAP_SENT_TAG_KEY = "vires-runner-bootstrap-sent"
BOOTSTRAP_DEADLINE_SECONDS = int(os.environ.get("VIRES_RUNNER_BOOTSTRAP_DEADLINE_SECONDS", "300"))
RUNNER_MAX_LIFETIME_SECONDS = int(os.environ.get("VIRES_RUNNER_MAX_LIFETIME_SECONDS", "5400"))
...
TELEGRAM_BOT_TOKEN_SSM = os.environ.get("VIRES_RUNNER_TELEGRAM_BOT_TOKEN_SSM",
                                        "/alpha-engine/TELEGRAM_BOT_TOKEN")
TELEGRAM_CHAT_ID_SSM = os.environ.get("VIRES_RUNNER_TELEGRAM_CHAT_ID_SSM",
...
WEBHOOK_SECRET_SSM = os.environ.get(
    "VIRES_RUNNER_WEBHOOK_SECRET_SSM", "/vires/runner/webhook_secret"
)
...
VIRES_RUNNER_GH_PAT_SSM = os.environ.get(
    "VIRES_RUNNER_GH_PAT_SSM", "/vires/runner/github_pat"
)
VIRES_RUNNER_CONFIG_BRANCH = os.environ.get("VIRES_RUNNER_CONFIG_BRANCH", "main")
MAX_RUNTIME_SECONDS = int(os.environ.get("VIRES_RUNNER_MAX_RUNTIME_SECONDS", "1800"))
...
CW_LOG_GROUP = os.environ.get("VIRES_RUNNER_CW_LOG_GROUP", "/vires/runner-spot")
...
RECONCILE_STALE_SECONDS = int(os.environ.get("VIRES_RUNNER_RECONCILE_STALE_SECONDS", "90"))
RECONCILE_MAX_QUEUED_RUNS = int(os.environ.get("VIRES_RUNNER_RECONCILE_MAX_QUEUED_RUNS", "50"))
...
# Default 20 concurrent vires-runner boxes (t3.medium-class, 2 vCPU each =
...
MAX_CONCURRENT_RUNNERS = int(os.environ.get("VIRES_RUNNER_MAX_CONCURRENT", "20"))

VIRES_RUNNER_READ_PAT_SSM = os.environ.get(
    "VIRES_RUNNER_READ_PAT_SSM", "/vires/runner/github_read_pat"
)
...
def _gh_read_pat() -> str:
    ...
    if _gh_read_pat_cache is None:
        _gh_read_pat_cache = boto3.client("ssm", region_name=REGION).get_parameter(
            Name=VIRES_RUNNER_READ_PAT_SSM, WithDecryption=True  # gitleaks:allow — SSM param path, not a secret value
        )["Parameter"]["Value"]
...
def _handle_webhook(event: dict) -> dict:
    ...
    if not DISPATCH_ENABLED:
        logger.warning("VIRES_RUNNER_DISPATCH_ENABLED=false — job %s NOT dispatched", job_id)
    ...
    boto3.client("lambda", region_name=REGION).invoke(
        ...
        Payload=json.dumps({"vires_runner_job_id": str(job_id)}).encode("utf-8"),
    )
    return _response(200, {"accepted": True, "job_id": job_id})


def _running_vires_runner_instance_ids(job_id: str) -> list[str]:
    return spot_dispatch.running_instance_ids(
        VIRES_RUNNER_TAG_NAME,
        {VIRES_RUNNER_JOB_ID_TAG_KEY: job_id},
        region=REGION,
    )
...
def _job_attempt_count(job_id: str) -> int:
    """How many vires-runner boxes have been launched for ``job_id`` within
    the last ``VIRES_RUNNER_ATTEMPT_LOOKBACK_SECONDS`` (live or recently-
    ...
    posture; see _launch_vires_runner_spot)."""
    ec2 = boto3.client("ec2", region_name=REGION)
    resp = ec2.describe_instances(Filters=[
        {"Name": "tag:Name", "Values": [VIRES_RUNNER_TAG_NAME]},
        {"Name": f"tag:{VIRES_RUNNER_JOB_ID_TAG_KEY}", "Values": [job_id]},
    ...
    for r in resp.get("Reservations", []):
        for i in r.get("Instances", []):
            ...
            if launch_time is not None:
                ...
                if age > VIRES_RUNNER_ATTEMPT_LOOKBACK_SECONDS:
                    continue
            count += 1
    return count
...
def _current_fleet_size() -> int:
    """Running+pending vires-runner boxes, fleet-wide (across ALL jobs) —
...
VIRES_RUNNER_LAUNCH_METRIC_NAMESPACE = "AlphaEngine/Infra"
VIRES_RUNNER_LAUNCH_METRIC_NAME = "vires_runner_launches"
...
def _emit_launch_metric() -> None:
    ...
    infrastructure/setup_vires_runner_launch_rate_alarm.sh (this repo has no
    ...
    try:
        boto3.client("cloudwatch", region_name=REGION).put_metric_data(
            Namespace=VIRES_RUNNER_LAUNCH_METRIC_NAMESPACE,
            MetricData=[{
                "MetricName": VIRES_RUNNER_LAUNCH_METRIC_NAME,
                "Value": 1.0,
                "Unit": "Count",
            }],
        )
...
export HOME=/root
fail() {{ echo "[vires-runner-prelude] FATAL: $1"; shutdown -h now; exit 1; }}
dnf install -y -q git python3.12 >/dev/null 2>&1 || fail "runtime install (git/python3.12) failed"
PAT=$(aws ssm get-parameter --name {VIRES_RUNNER_GH_PAT_SSM} --with-decryption \
  --query Parameter.Value --output text --region {REGION} 2>/dev/null) || fail "PAT read failed"
[ -n "$PAT" ] || fail "PAT empty"
git config --global --add safe.directory '*' || true
rm -rf /home/ec2-user/vires
git clone --depth 1 --branch {VIRES_RUNNER_CONFIG_BRANCH} \
  "https://x-access-token:${{PAT}}@github.com/{TARGET_REPO_FULL_NAME}.git" \
  /home/ec2-user/vires || fail "clone failed"
cd /home/ec2-user/vires
exec bash infrastructure/vires_runner_spot_bootstrap.sh --job-id "{job_id}"
"""


def _launch_vires_runner_spot(job_id: str) -> dict:
    ...
    try:
        ...
        logger.error(
            "vires-runner attempt-count probe FAILED for job %s — refusing to "
    ...
    if attempt_count >= VIRES_RUNNER_MAX_ATTEMPTS_PER_JOB:
        if job_id not in _abandoned_job_ids_paged:
            _abandoned_job_ids_paged.add(job_id)
            _page(
                f"🔴 vires-runner: job {job_id} ABANDONED after "
                f"{attempt_count} launch attempts (limit "
                f"{VIRES_RUNNER_MAX_ATTEMPTS_PER_JOB}) — every box likely "
        ...
        logger.error(
            "vires-runner job %s ABANDONED — %d attempts >= limit %d, "
            "not dispatching", job_id, attempt_count, VIRES_RUNNER_MAX_ATTEMPTS_PER_JOB,
        )
    ...
    try:
        ...
        logger.error(
            "vires-runner fleet-size probe FAILED for job %s — refusing to "
            "dispatch (fail-safe): %s: %s", job_id, type(exc).__name__, exc,
        )
        return {"launched": False, "reason": "fleet_probe_failed", "job_id": job_id,
                "error": str(exc)}

    if fleet_size >= VIRES_RUNNER_MAX_FLEET:
        _page(
            f"🔴 vires-runner: fleet cap reached ({fleet_size} >= "
            f"{VIRES_RUNNER_MAX_FLEET}) — job {job_id} NOT dispatched this "
        ...
        logger.error(
            "vires-runner FLEET CAP reached (%d >= %d) — job %s not "
            "dispatched", fleet_size, VIRES_RUNNER_MAX_FLEET, job_id,
        )
    ...
    try:
        existing = _running_vires_runner_instance_ids(job_id)
        ...
        existing = []
        logger.error(
            "vires-runner concurrency probe FAILED for job %s — proceeding "
            "with dedupe_degraded=true: %s: %s", job_id, type(exc).__name__, exc,
        )
    if existing:
        logger.warning("vires-runner box already live for job %s (%s) — skipping",
                       job_id, existing)
    ...
    try:
        instance_id, market = spot_dispatch.launch_with_fallback(
            ...
            tag_name=VIRES_RUNNER_TAG_NAME,
            region=REGION,
        )
    except SpotLaunchError as exc:
        logger.error("vires-runner spot launch failed for job %s: %s: %s",
                     job_id, type(exc).__name__, exc)
        if "MaxSpotInstanceCountExceeded" in str(exc):
            _page("🔴 vires-runner: spot QUOTA EXHAUSTED "
                  ...
                  f"Name={VIRES_RUNNER_TAG_NAME}) — the reconcile janitor "
    ...
    logger.info("launched vires-runner box %s (%s) for job %s%s", instance_id, market, job_id,
    ...
    try:
        boto3.client("ec2", region_name=REGION).create_tags(
            ...
            Tags=[{"Key": VIRES_RUNNER_JOB_ID_TAG_KEY, "Value": job_id}],
        )
    except Exception as exc:  # noqa: BLE001 — load-bearing tag write; terminate + fail loud on failure
        spot_dispatch.terminate_on_failure(instance_id, region=REGION, label="vires-runner")
        logger.error("vires-runner discriminator tag write FAILED for %s (job %s) — "
    ...
    logger.info("vires-runner launched (bootstrap deferred to reconcile): "
...
def _bootstrap_and_reap() -> dict:
    ...
    running vires-runner box, exactly one of:
...
def _reconcile() -> dict:
    ...
    job instead."""
    if not DISPATCH_ENABLED:
        logger.info("reconcile: VIRES_RUNNER_DISPATCH_ENABLED=false — skipping")
    ...
    for age_seconds, job_id in sorted(stale_jobs, reverse=True):  # oldest first
        ...
        logger.info("reconcile: job %s queued %.0fs with no in-flight box — dispatching",
                   job_id, age_seconds)
        result = _launch_vires_runner_spot(job_id)
        if result.get("launched"):
            available_slots -= 1
        dispatched.append({"job_id": job_id, "age_seconds": age_seconds, "result": result})

    if capped_at_slot_zero:
        _page(f"⚠️ vires-runner: MAX_CONCURRENT_RUNNERS={MAX_CONCURRENT_RUNNERS} reached — "
...
def handler(event: dict, context) -> dict:
    ...
    job_id = event.get("vires_runner_job_id")
    if not job_id:
        logger.error("worker-phase invocation missing vires_runner_job_id: %r", event)
        return {"launched": False, "reason": "invalid_event"}
    return _launch_vires_runner_spot(str(job_id))
