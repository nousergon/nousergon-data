...
moved the read-only WIRING checks to overseer-liveness-probe:
  ...
  * the mid-run spot-reclaim checker (config#2270) — EC2 reclaim/termination
    events -> exactly-one bounded on-demand relaunch, drill isolation, escalation;
  * the disabled-window dropped-failure sweep (config#2257) — the scheduled
    invocation re-drives an uncovered terminal failure through the saturday
    dispatcher, gated on the spot dispatcher's live SF_WATCH_DISPATCH_ENABLED.

Mocks flow-doctor notify (no live Telegram) and boto3 stepfunctions/lambda/s3/ec2.
"""
...
def _healthy_event_pattern() -> str:
    # The slimmed handler never inspects the EventBridge rule; the reclaim/sweep
    # client factories only need a placeholder events client, so a static minimal
    # pattern suffices (no dependency on the now-moved wiring constants).
    return json.dumps({
        "source": ["aws.states"],
...
def test_scheduled_probe_event_never_routes_to_reclaim_branch():
    """The scheduled probe payload is {} — it must run the sweep path, not the
    reclaim checker; this pins the event-shape discriminator directly."""
