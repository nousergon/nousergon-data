#!/usr/bin/env bash
# deploy.sh — Create or update the metron-runner-dispatcher Lambda.
#
# WHY: mirrors nousergon/alpha-engine-config's self-hosted-runner dispatcher
# (alpha-engine-config-I2572) verbatim in mechanism, re-namespaced for metron.
# A live billing-usage audit (2026-07-17) found metron is the largest
# non-alpha-engine-config private-repo GHA-minute consumer (~2,348 min in
# July alone, on ci.yml's pytest+ruff+web-build jobs and Dependabot/CI churn)
# after the org crossed its 3,000-min included-Actions-minute line for the
# month. This Lambda receives GitHub's `workflow_job` webhook directly
...
# (scoped to metron-runner-executor-role — a NEW, dedicated role
# a sibling agent is creating in metron) + ssm:SendCommand +
...
#        /metron/runner/webhook_secret  (random string, shared
#          with the GitHub webhook config below)
#        /metron/runner/github_pat  (fine-grained PAT, owner=
#          nousergon, repo=metron, Administration: Read and
...
#        /metron/runner/github_read_pat  (SEPARATE fine-grained
#          PAT, owner=nousergon, repo=metron, Actions: Read
...
#   2. Register the GitHub webhook on nousergon/metron:
#        event: workflow_job, content type: json, secret: (the same value as
#        the SSM param above), URL: this script's printed Function URL.
#      Can be done via `gh api repos/nousergon/metron/hooks`.
#
# Usage:
#   bash .../metron-runner-dispatcher/deploy.sh             # update code only (also the CI auto-deploy path)
#   bash .../metron-runner-dispatcher/deploy.sh --bootstrap # operator-only: create/update the IAM role + Lambda + Function URL + reconcile schedule
#   bash .../metron-runner-dispatcher/deploy.sh --apply-iam # re-apply iam-policy.json only (no bootstrap side effects, config#2825)
#   bash .../metron-runner-dispatcher/deploy.sh --dry-run   # show actions, do not apply
#   bash .../metron-runner-dispatcher/deploy.sh --smoke     # invoke the WORKER phase once with a synthetic job id (fires a REAL spot box)
#   bash .../metron-runner-dispatcher/deploy.sh --reconcile-test # invoke the RECONCILE phase once directly (read-only unless it finds a genuinely stale job)

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/../_shared/apply_iam_policy.sh"
FUNCTION_NAME="metron-runner-dispatcher"
ROLE_NAME="metron-runner-dispatcher-role"
POLICY_NAME="metron-runner-dispatcher-policy"
...
LAMBDA_ENV_BOOTSTRAP='Variables={LOG_LEVEL=INFO,METRON_RUNNER_DISPATCH_ENABLED=true}'
...
# instead of silently running the real deploy path (metron-
...
if $BOOTSTRAP; then
  ...
  if ! $DRY_RUN; then
    ...
    echo "  -> register as the GitHub webhook target for nousergon/metron"
    echo "     (event: workflow_job, content type: json, secret: matches"
    echo "     /metron/runner/webhook_secret) — see this script's"
  ...
  RECONCILE_SCHED_ROLE_NAME="metron-runner-reconcile-scheduler-role"
  RECONCILE_SCHED_POLICY_NAME="invoke-metron-runner-dispatcher"
  ...
  RECONCILE_SCHED_NAME="metron-runner-reconcile"
...
echo "Updating Lambda environment (preserving operator-owned METRON_RUNNER_DISPATCH_ENABLED)..."
CURRENT_DISPATCH=$(preserve_env_flag "${FUNCTION_NAME}" "${REGION}" METRON_RUNNER_DISPATCH_ENABLED true)
LAMBDA_ENV="Variables={LOG_LEVEL=INFO,METRON_RUNNER_DISPATCH_ENABLED=${CURRENT_DISPATCH}}"
...
if $SMOKE; then
  ...
  echo "⚠ this fires a REAL spot box + REAL metron_runner_spot_bootstrap.sh run."
  RESP=$(mktemp)
  trap "rm -f '${RESP}'" EXIT
  aws lambda invoke \
    --function-name "${FUNCTION_NAME}" \
    --payload '{"metron_runner_job_id":"smoke-test-0"}' \
