"""metron-runner-dispatcher — launch an EPHEMERAL GitHub Actions
self-hosted runner on a dedicated EC2 spot box, once per queued CI/validation
job in nousergon/metron. Mirrors nousergon/alpha-engine-config's
self-hosted-runner dispatcher (alpha-engine-config-I2572) verbatim in
mechanism, re-namespaced for metron.

WHY: a hard GHA audit (2026-07-14) found metron's own CI/
...
`handler` for the branch):
  1. WEBHOOK RECEIVER phase: GitHub calls this Lambda's Function URL directly
     on every `workflow_job` event for metron (repo-level
     webhook, event type `workflow_job`). Verifies the HMAC signature, filters
     to `action=queued` + our `metron-spot` label, then
  ...
  2. WORKER phase (the self-invoked async call): does the actual dispatch —
     ...
     command that clones metron and `exec`s
     `infrastructure/metron_runner_spot_bootstrap.sh` (built by a sibling
     agent in metron) — which installs+registers the actual
  ...
  3. RECONCILE phase (EventBridge Scheduler, ~every 60s — config-I2653): a
     ...
     longer than `METRON_RUNNER_RECONCILE_STALE_SECONDS` with no in-flight
...
total fleet size. Both live in `_launch_metron_runner_spot()` (the single
...
a job at/over `METRON_RUNNER_MAX_ATTEMPTS_PER_JOB` launches (default 3,
...
capped at `METRON_RUNNER_MAX_FLEET` running+pending boxes (default 6 = 12
...
metron_runner_launches` custom metric + CloudWatch alarm
(infrastructure/setup_metron_runner_launch_rate_alarm.sh) is the
...
CONCURRENCY LOCK: keyed on `Name=metron-runner-spot` +
`metron-runner-job-id=<workflow_job.id>` — one job, one box, 1:1 (unlike
...
IAM PROFILE: `metron-runner-executor-profile`, a NEW dedicated
instance profile (infrastructure/iam/metron-runner-executor-role-*.json in
metron) scoped to read exactly one SSM param (the runner-
...
with Administration:write on metron exists in SSM — see
...
# Kill-switch: METRON_RUNNER_DISPATCH_ENABLED=false disables the launch
# without touching the GitHub webhook wiring — mirrors every other fleet
# dispatcher's safety valve. Default ON.
DISPATCH_ENABLED = os.environ.get("METRON_RUNNER_DISPATCH_ENABLED", "true").lower() == "true"

TARGET_REPO_FULL_NAME = "nousergon/metron"
TARGET_LABEL = "metron-spot"
...
INSTANCE_TYPES = [
    ...
    for t in os.environ.get(
        "METRON_RUNNER_INSTANCE_TYPES", "t3.medium,t3a.medium,t2.medium"
    ).split(",")
    if t.strip()
]
SUBNETS = [
    ...
    for s in os.environ.get(
        "METRON_RUNNER_SUBNETS",
        "subnet-a61ec0fb,subnet-1e58307a,subnet-789d3857,"
        "subnet-c670118d,subnet-7cff7c43,subnet-e07166ec",
    ).split(",")
    if s.strip()
]
AMI_ID = os.environ.get("METRON_RUNNER_AMI_ID", "ami-0c421724a94bba6d6")  # Amazon Linux 2023 x86_64
KEY_NAME = os.environ.get("METRON_RUNNER_KEY_NAME", "alpha-engine-key")
SECURITY_GROUP = os.environ.get("METRON_RUNNER_SECURITY_GROUP", "sg-03cd3c4bd91e610b0")
IAM_PROFILE = os.environ.get(
    "METRON_RUNNER_IAM_PROFILE", "metron-runner-executor-profile"
)
VOLUME_SIZE_GB = int(os.environ.get("METRON_RUNNER_VOLUME_SIZE_GB", "30"))

METRON_RUNNER_TAG_NAME = "metron-runner-spot"
METRON_RUNNER_JOB_ID_TAG_KEY = "metron-runner-job-id"
...
# inside _launch_metron_runner_spot (the single chokepoint both the reactive
...
#      running+pending metron-runner boxes >= MAX_FLEET, regardless of
...
METRON_RUNNER_MAX_ATTEMPTS_PER_JOB = int(os.environ.get("METRON_RUNNER_MAX_ATTEMPTS_PER_JOB", "3"))
...
METRON_RUNNER_ATTEMPT_LOOKBACK_SECONDS = int(
    os.environ.get("METRON_RUNNER_ATTEMPT_LOOKBACK_SECONDS", "3600")
)
...
METRON_RUNNER_MAX_FLEET = int(os.environ.get("METRON_RUNNER_MAX_FLEET", "6"))
...
# silently again).
METRON_RUNNER_BOOTSTRAP_SENT_TAG_KEY = "metron-runner-bootstrap-sent"
BOOTSTRAP_DEADLINE_SECONDS = int(os.environ.get("METRON_RUNNER_BOOTSTRAP_DEADLINE_SECONDS", "300"))
RUNNER_MAX_LIFETIME_SECONDS = int(os.environ.get("METRON_RUNNER_MAX_LIFETIME_SECONDS", "5400"))
...
TELEGRAM_BOT_TOKEN_SSM = os.environ.get("METRON_RUNNER_TELEGRAM_BOT_TOKEN_SSM",
                                        "/alpha-engine/TELEGRAM_BOT_TOKEN")
TELEGRAM_CHAT_ID_SSM = os.environ.get("METRON_RUNNER_TELEGRAM_CHAT_ID_SSM",
...
WEBHOOK_SECRET_SSM = os.environ.get(
    "METRON_RUNNER_WEBHOOK_SECRET_SSM", "/metron/runner/webhook_secret"
)
...
METRON_RUNNER_GH_PAT_SSM = os.environ.get(
    "METRON_RUNNER_GH_PAT_SSM", "/metron/runner/github_pat"
)
METRON_RUNNER_CONFIG_BRANCH = os.environ.get("METRON_RUNNER_CONFIG_BRANCH", "main")
MAX_RUNTIME_SECONDS = int(os.environ.get("METRON_RUNNER_MAX_RUNTIME_SECONDS", "1800"))
...
CW_LOG_GROUP = os.environ.get("METRON_RUNNER_CW_LOG_GROUP", "/metron/runner-spot")
...
RECONCILE_STALE_SECONDS = int(os.environ.get("METRON_RUNNER_RECONCILE_STALE_SECONDS", "90"))
RECONCILE_MAX_QUEUED_RUNS = int(os.environ.get("METRON_RUNNER_RECONCILE_MAX_QUEUED_RUNS", "50"))
...
# Default 20 concurrent metron-runner boxes (t3.medium-class, 2 vCPU each =
...
MAX_CONCURRENT_RUNNERS = int(os.environ.get("METRON_RUNNER_MAX_CONCURRENT", "20"))

METRON_RUNNER_READ_PAT_SSM = os.environ.get(
    "METRON_RUNNER_READ_PAT_SSM", "/metron/runner/github_read_pat"
)
...
def _gh_read_pat() -> str:
    ...
    if _gh_read_pat_cache is None:
        _gh_read_pat_cache = boto3.client("ssm", region_name=REGION).get_parameter(
            Name=METRON_RUNNER_READ_PAT_SSM, WithDecryption=True  # gitleaks:allow — SSM param path, not a secret value
        )["Parameter"]["Value"]
...
def _handle_webhook(event: dict) -> dict:
    ...
    if not DISPATCH_ENABLED:
        logger.warning("METRON_RUNNER_DISPATCH_ENABLED=false — job %s NOT dispatched", job_id)
    ...
    boto3.client("lambda", region_name=REGION).invoke(
        ...
        Payload=json.dumps({"metron_runner_job_id": str(job_id)}).encode("utf-8"),
    )
    return _response(200, {"accepted": True, "job_id": job_id})


def _running_metron_runner_instance_ids(job_id: str) -> list[str]:
    return spot_dispatch.running_instance_ids(
        METRON_RUNNER_TAG_NAME,
        {METRON_RUNNER_JOB_ID_TAG_KEY: job_id},
        region=REGION,
    )
...
def _job_attempt_count(job_id: str) -> int:
    """How many metron-runner boxes have been launched for ``job_id`` within
    the last ``METRON_RUNNER_ATTEMPT_LOOKBACK_SECONDS`` (live or recently-
    ...
    posture; see _launch_metron_runner_spot)."""
    ec2 = boto3.client("ec2", region_name=REGION)
    resp = ec2.describe_instances(Filters=[
        {"Name": "tag:Name", "Values": [METRON_RUNNER_TAG_NAME]},
        {"Name": f"tag:{METRON_RUNNER_JOB_ID_TAG_KEY}", "Values": [job_id]},
    ...
    for r in resp.get("Reservations", []):
        for i in r.get("Instances", []):
            ...
            if launch_time is not None:
                ...
                if age > METRON_RUNNER_ATTEMPT_LOOKBACK_SECONDS:
                    continue
            count += 1
    return count
...
def _current_fleet_size() -> int:
    """Running+pending metron-runner boxes, fleet-wide (across ALL jobs) —
...
METRON_RUNNER_LAUNCH_METRIC_NAMESPACE = "AlphaEngine/Infra"
METRON_RUNNER_LAUNCH_METRIC_NAME = "metron_runner_launches"
...
def _emit_launch_metric() -> None:
    ...
    infrastructure/setup_metron_runner_launch_rate_alarm.sh (this repo has no
    ...
    try:
        boto3.client("cloudwatch", region_name=REGION).put_metric_data(
            Namespace=METRON_RUNNER_LAUNCH_METRIC_NAMESPACE,
            MetricData=[{
                "MetricName": METRON_RUNNER_LAUNCH_METRIC_NAME,
                "Value": 1.0,
                "Unit": "Count",
            }],
        )
...
export HOME=/root
fail() {{ echo "[metron-runner-prelude] FATAL: $1"; shutdown -h now; exit 1; }}
dnf install -y -q git python3.12 >/dev/null 2>&1 || fail "runtime install (git/python3.12) failed"
PAT=$(aws ssm get-parameter --name {METRON_RUNNER_GH_PAT_SSM} --with-decryption \
  --query Parameter.Value --output text --region {REGION} 2>/dev/null) || fail "PAT read failed"
[ -n "$PAT" ] || fail "PAT empty"
git config --global --add safe.directory '*' || true
rm -rf /home/ec2-user/metron
git clone --depth 1 --branch {METRON_RUNNER_CONFIG_BRANCH} \
  "https://x-access-token:${{PAT}}@github.com/{TARGET_REPO_FULL_NAME}.git" \
  /home/ec2-user/metron || fail "clone failed"
cd /home/ec2-user/metron
exec bash infrastructure/metron_runner_spot_bootstrap.sh --job-id "{job_id}"
"""


def _launch_metron_runner_spot(job_id: str) -> dict:
    ...
    try:
        ...
        logger.error(
            "metron-runner attempt-count probe FAILED for job %s — refusing to "
    ...
    if attempt_count >= METRON_RUNNER_MAX_ATTEMPTS_PER_JOB:
        if job_id not in _abandoned_job_ids_paged:
            _abandoned_job_ids_paged.add(job_id)
            _page(
                f"🔴 metron-runner: job {job_id} ABANDONED after "
                f"{attempt_count} launch attempts (limit "
                f"{METRON_RUNNER_MAX_ATTEMPTS_PER_JOB}) — every box likely "
        ...
        logger.error(
            "metron-runner job %s ABANDONED — %d attempts >= limit %d, "
            "not dispatching", job_id, attempt_count, METRON_RUNNER_MAX_ATTEMPTS_PER_JOB,
        )
    ...
    try:
        ...
        logger.error(
            "metron-runner fleet-size probe FAILED for job %s — refusing to "
            "dispatch (fail-safe): %s: %s", job_id, type(exc).__name__, exc,
        )
        return {"launched": False, "reason": "fleet_probe_failed", "job_id": job_id,
                "error": str(exc)}

    if fleet_size >= METRON_RUNNER_MAX_FLEET:
        _page(
            f"🔴 metron-runner: fleet cap reached ({fleet_size} >= "
            f"{METRON_RUNNER_MAX_FLEET}) — job {job_id} NOT dispatched this "
        ...
        logger.error(
            "metron-runner FLEET CAP reached (%d >= %d) — job %s not "
            "dispatched", fleet_size, METRON_RUNNER_MAX_FLEET, job_id,
        )
    ...
    try:
        existing = _running_metron_runner_instance_ids(job_id)
        ...
        existing = []
        logger.error(
            "metron-runner concurrency probe FAILED for job %s — proceeding "
            "with dedupe_degraded=true: %s: %s", job_id, type(exc).__name__, exc,
        )
    if existing:
        logger.warning("metron-runner box already live for job %s (%s) — skipping",
                       job_id, existing)
    ...
    try:
        instance_id, market = spot_dispatch.launch_with_fallback(
            ...
            tag_name=METRON_RUNNER_TAG_NAME,
            region=REGION,
        )
    except SpotLaunchError as exc:
        logger.error("metron-runner spot launch failed for job %s: %s: %s",
                     job_id, type(exc).__name__, exc)
        if "MaxSpotInstanceCountExceeded" in str(exc):
            _page("🔴 metron-runner: spot QUOTA EXHAUSTED "
                  ...
                  f"Name={METRON_RUNNER_TAG_NAME}) — the reconcile janitor "
    ...
    logger.info("launched metron-runner box %s (%s) for job %s%s", instance_id, market, job_id,
    ...
    try:
        boto3.client("ec2", region_name=REGION).create_tags(
            ...
            Tags=[{"Key": METRON_RUNNER_JOB_ID_TAG_KEY, "Value": job_id}],
        )
    except Exception as exc:  # noqa: BLE001 — load-bearing tag write; terminate + fail loud on failure
        spot_dispatch.terminate_on_failure(instance_id, region=REGION, label="metron-runner")
        logger.error("metron-runner discriminator tag write FAILED for %s (job %s) — "
    ...
    logger.info("metron-runner launched (bootstrap deferred to reconcile): "
...
def _bootstrap_and_reap() -> dict:
    ...
    running metron-runner box, exactly one of:
...
def _reconcile() -> dict:
    ...
    job instead."""
    if not DISPATCH_ENABLED:
        logger.info("reconcile: METRON_RUNNER_DISPATCH_ENABLED=false — skipping")
    ...
    for age_seconds, job_id in sorted(stale_jobs, reverse=True):  # oldest first
        ...
        logger.info("reconcile: job %s queued %.0fs with no in-flight box — dispatching",
                   job_id, age_seconds)
        result = _launch_metron_runner_spot(job_id)
        if result.get("launched"):
            available_slots -= 1
        dispatched.append({"job_id": job_id, "age_seconds": age_seconds, "result": result})

    if capped_at_slot_zero:
        _page(f"⚠️ metron-runner: MAX_CONCURRENT_RUNNERS={MAX_CONCURRENT_RUNNERS} reached — "
...
def handler(event: dict, context) -> dict:
    ...
    job_id = event.get("metron_runner_job_id")
    if not job_id:
        logger.error("worker-phase invocation missing metron_runner_job_id: %r", event)
        return {"launched": False, "reason": "invalid_event"}
    return _launch_metron_runner_spot(str(job_id))
