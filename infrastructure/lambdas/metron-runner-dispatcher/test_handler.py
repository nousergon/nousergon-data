...
class _FakeEc2:
    ...
    def describe_instances(self, Filters):  # noqa: N803
        ...
        job_id_filter = by_name.get("tag:metron-runner-job-id")

        # config#2697 _job_attempt_count: tag:Name + tag:metron-runner-job-id
...
class _FakeSsm:
    def __init__(self):
        self.sent = []
        self.params = {
            "/metron/runner/webhook_secret": WEBHOOK_SECRET,
            "/metron/runner/github_read_pat": "test-read-only-pat",
        }
...
def _load(monkeypatch, *, launch_impl=None, env=None, running_job_ids=None,
    ...
    monkeypatch.setenv("AWS_LAMBDA_FUNCTION_NAME", "metron-runner-dispatcher")
...
def _job_payload(**overrides):
    base = {
        "action": "queued",
        "repository": {"full_name": "nousergon/metron"},
        "workflow_job": {"id": 987654321, "labels": ["self-hosted", "metron-spot"]},
    }
...
def test_webhook_valid_queued_job_self_invokes_and_returns_200(monkeypatch):
    idx = _load(monkeypatch, env={"METRON_RUNNER_DISPATCH_ENABLED": "true"})
    ...
    assert inv["FunctionName"] == "metron-runner-dispatcher"
    worker_payload = json.loads(inv["Payload"])
    assert worker_payload == {"metron_runner_job_id": "987654321"}
...
def test_webhook_disabled_flag_skips_invoke(monkeypatch):
    idx = _load(monkeypatch, env={"METRON_RUNNER_DISPATCH_ENABLED": "false"})
    resp = idx.handler(_webhook_event(_job_payload()), None)
    assert resp["statusCode"] == 200
    body = json.loads(resp["body"])
...
def test_worker_valid_job_launches_tags_and_defers_bootstrap(monkeypatch):
    ...
    idx = _load(monkeypatch, launch_impl=_launch, env={"METRON_RUNNER_DISPATCH_ENABLED": "true"})
    out = idx.handler({"metron_runner_job_id": "987654321"}, None)
    assert out["launched"] is True
    assert out["reason"] == "launched_bootstrap_pending"
    assert out["instance_id"] == "i-abc"
    assert out["market"] == "spot"
    assert calls["profile"] == "metron-runner-executor-profile"
    assert calls["tag_name"] == "metron-runner-spot"
    assert idx._test_ec2.tags_created == [
        (["i-abc"], [{"Key": "metron-runner-job-id", "Value": "987654321"}])
    ]
...
def test_worker_on_demand_fallback_on_spot_capacity_exhaustion(monkeypatch):
    ...
    idx = _load(monkeypatch, launch_impl=_launch, env={"METRON_RUNNER_DISPATCH_ENABLED": "true"})
    out = idx.handler({"metron_runner_job_id": "1"}, None)
    assert out["launched"] is True
...
def test_worker_concurrency_skip_when_job_already_running(monkeypatch):
    ...
    idx = _load(
        monkeypatch, launch_impl=_launch, env={"METRON_RUNNER_DISPATCH_ENABLED": "true"},
        running_job_ids={"55": ["i-already-running"]},
    )
    out = idx.handler({"metron_runner_job_id": "55"}, None)
    assert out["launched"] is False
...
def test_worker_different_job_id_is_not_blocked(monkeypatch):
    idx = _load(
        ...
        env={"METRON_RUNNER_DISPATCH_ENABLED": "true"},
        running_job_ids={"55": ["i-other-job"]},
    )
    out = idx.handler({"metron_runner_job_id": "56"}, None)
    assert out["launched"] is True
...
def _box(iid, *, age_seconds, job_id="j1", bootstrapped=False):
    tags = [{"Key": "Name", "Value": "metron-runner-spot"}]
    if job_id:
        tags.append({"Key": "metron-runner-job-id", "Value": job_id})
    if bootstrapped:
        tags.append({"Key": "metron-runner-bootstrap-sent", "Value": "2026-01-01T00:00:00Z"})
...
def test_bootstrap_and_reap_sends_bootstrap_to_online_untagged_box(monkeypatch):
    ...
    assert 'exec bash infrastructure/metron_runner_spot_bootstrap.sh --job-id "42"' in cmd
    assert any(r == ["i-new"] and tags[0]["Key"] == "metron-runner-bootstrap-sent"
...
def test_bootstrap_and_reap_send_failure_leaves_untagged_for_retry(monkeypatch):
    ...
    assert all(tags[0]["Key"] != "metron-runner-bootstrap-sent"
    ...
    _install_gh_api_stub(idx, monkeypatch, {
        ...
        "/actions/runs/555/jobs": {
            "jobs": [{
                ...
                "labels": ["self-hosted", "metron-spot"],
                "created_at": _iso(200),
            }],
        },
    })
...
def test_reconcile_skips_job_already_covered_by_an_in_flight_box(monkeypatch):
    idx = _load(
        monkeypatch, env={"METRON_RUNNER_DISPATCH_ENABLED": "true"},
...
def test_reconcile_respects_concurrency_cap_leaves_job_queued(monkeypatch):
    idx = _load(monkeypatch, env={"METRON_RUNNER_DISPATCH_ENABLED": "true",
                                    "METRON_RUNNER_MAX_CONCURRENT": "1"})
...
# _launch_metron_runner_spot (the single chokepoint both the reactive
...
def test_attempt_limit_blocks_dispatch_at_default_threshold(monkeypatch):
    # Default METRON_RUNNER_MAX_ATTEMPTS_PER_JOB=3: 3 prior (terminated)
    ...
    out = idx.handler({"metron_runner_job_id": "999"}, None)
    assert out["launched"] is False
...
def test_attempt_limit_pages_exactly_once_per_job(monkeypatch):
    ...
    pages = []
    monkeypatch.setattr(idx, "_page", lambda msg: pages.append(msg))
    idx.handler({"metron_runner_job_id": "42"}, None)
    idx.handler({"metron_runner_job_id": "42"}, None)
    idx.handler({"metron_runner_job_id": "42"}, None)
...
def test_attempt_count_below_threshold_still_dispatches(monkeypatch):
    ...
    out = idx.handler({"metron_runner_job_id": "5"}, None)
    assert out["launched"] is True
...
def test_attempt_count_ignores_boxes_outside_lookback_window(monkeypatch):
    ...
    out = idx.handler({"metron_runner_job_id": "7"}, None)
    assert out["launched"] is True
...
def test_attempt_limit_is_env_tunable(monkeypatch):
    ...
    idx = _load(
        ...
        env={"METRON_RUNNER_DISPATCH_ENABLED": "true", "METRON_RUNNER_MAX_ATTEMPTS_PER_JOB": "1"},
        attempt_boxes={"9": [_attempt_box("i-only-prior-attempt", age_seconds=10)]},
    )
    out = idx.handler({"metron_runner_job_id": "9"}, None)
    assert out["launched"] is False
    assert out["reason"] == "attempt_limit_exceeded"
    assert launched == []
...
def test_attempt_probe_failure_is_fail_safe_not_fail_open(monkeypatch):
    ...
    out = idx.handler({"metron_runner_job_id": "123"}, None)
    assert out["launched"] is False
...
def test_fleet_cap_blocks_dispatch_at_default_threshold(monkeypatch):
    # Default METRON_RUNNER_MAX_FLEET=6: 6 running+pending boxes already
    ...
    out = idx.handler({"metron_runner_job_id": "888"}, None)
    assert out["launched"] is False
...
def test_fleet_cap_pages(monkeypatch):
    ...
    pages = []
    monkeypatch.setattr(idx, "_page", lambda msg: pages.append(msg))
    idx.handler({"metron_runner_job_id": "1"}, None)
    assert len(pages) == 1
...
def test_fleet_cap_is_env_tunable(monkeypatch):
    ...
    idx = _load(
        monkeypatch, launch_impl=_launch,
        env={"METRON_RUNNER_DISPATCH_ENABLED": "true", "METRON_RUNNER_MAX_FLEET": "1"},
    )
...
def test_fleet_cap_checked_even_when_this_job_has_zero_prior_attempts(monkeypatch):
    ...
    out = idx.handler({"metron_runner_job_id": "brand-new-job"}, None)
    assert out["launched"] is False
    assert out["reason"] == "fleet_cap_reached"
...
def test_reconcile_respects_attempt_limit_across_multiple_stale_jobs(monkeypatch):
    ...
    _install_gh_api_stub(idx, monkeypatch, {
        ...
        "/actions/runs/1/jobs": {
            "jobs": [
                {"id": 555, "status": "queued", "labels": ["self-hosted", "metron-spot"],
                 "created_at": _iso(200)},
                {"id": 556, "status": "queued", "labels": ["self-hosted", "metron-spot"],
                 "created_at": _iso(150)},
            ],
        },
    })
...
def test_successful_launch_emits_cloudwatch_launch_metric(monkeypatch):
    ...
    assert metric["MetricName"] == "metron_runner_launches"
