...
def test_soft_limit_min_override_forwarded_for_bounded_test(monkeypatch):
    # A manual invoke can bound today's test run (e.g. 60 min) without touching
    # any live schedule — none of the 3 SCHED_INPUTS carry this key.
    idx = _load(monkeypatch, env={"GROOM_DISPATCH_ENABLED": "true"})
    out = idx.handler({"run_mode": "full", "issue_filter": "high-only", "soft_limit_min": 60}, None)
