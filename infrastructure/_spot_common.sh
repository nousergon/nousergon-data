...
# reclaimed pool; final attempt on-demand) lives in ONE file, sourced by both
# this file and the spot_data_weekly.sh monolith (alpha-engine-config-I11565).
...
_spot_failure_reason() {
  ...
  # Record EVERY confirmed reclaim, including the one whose verdict is hold
  # (budget exhausted): the SF's DataPhase1Reissue re-runs this script from
  # attempt 1 on the same launcher box, and it must not walk back into the
  # pool that just failed (alpha-engine-config-I11565). stderr/file only.
