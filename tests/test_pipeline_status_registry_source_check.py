...
(config#1115, #1120, #2372, #2430, and the EOD instance this test's own
authoring surfaced — nousergon/nousergon-lib#201) — a new Task state lands
in one of THIS repo's SF JSONs without a paired
``nousergon_lib.pipeline_status.registry.STATE_TO_ARCHIVE_PAGE`` entry, and
it's always caught downstream in crucible-dashboard's CI, days/weeks late,
because the SF JSON that ADDS the new state lives HERE, not there.
...
later as a dashboard "⚠️ Registry drift" cell.
...
reported via a xfail-style skip-with-log rather than a hard assertion,
pending a separate registry-population pass for the groom pipeline. The
3 dashboard-covered files (Saturday/Weekday/EOD) ARE asserted on hard, per
this issue's scope.
"""
