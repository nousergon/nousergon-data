...
def _patch_targets(monkeypatch, *, universe_symbols: list[str], closes_tickers: list[str]):
    ...
    # I10704: every DECLARED benchmark proxy is collected daily (it is in
    # collectors/prices.py::_ALWAYS_DOWNLOAD), so it is present in closes —
    # IWM is the member the old literal sector_etfs list did not cover.
    from features.compute import UNIVERSE_BENCHMARK_PROXIES as _PROXIES_IN_CLOSES

    closes = _stub_closes(
        closes_tickers + macro_keys + sector_etfs + sorted(_PROXIES_IN_CLOSES)
    )
